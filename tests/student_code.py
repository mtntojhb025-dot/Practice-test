...
def calculate_cumulative_performance(scores: dict):
    ...
    return {"average": average, "below_average": list(filter(lambda e: scores[e] < average, scores))}
    pass
...
def analyze_improvement(scores: list):
    ...
    if scores[-1] > scores[0]:
        ...
        t = "negative"
    return {"trend": t, "improved": scores[-1] > scores[0]}
    pass
...
def rank_students(students: list[tuple[str, int]]):
    ...
    """
    return sorted(students, key = lambda a: a[-1])[::-1]
    pass
...
def sum_multiples_of_num(num: int, length: int):
    ...
    """
    return sum(list(filter(lambda a: a%num == 0, range(num, num*length+1, num))))
    pass
